...
#define MMC_SELECT()     GPIO_ResetBits(GPIOE, GPIO_Pin_7)       /* MMC CS = L */ 
#define MMC_DESELECT()   GPIO_SetBits(GPIOE, GPIO_Pin_7)         /* MMC CS = H */
...
void TurnToSD(void)
{
	...
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOA|RCC_APB2Periph_GPIOB|RCC_APB2Periph_GPIOE|RCC_APB2Periph_SPI1,ENABLE);

	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_7;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; // SD片选
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(GPIOE, &GPIO_InitStructure);
	GPIO_SetBits(GPIOE,GPIO_Pin_8); //PE7上拉
	
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_2;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; //PB2 245片选
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(GPIOB, &GPIO_InitStructure);
	GPIO_SetBits(GPIOB, GPIO_Pin_2);//关闭245
...
byte2 数据的长度
byte4 控制命令
byte5 byte6 byte7 byte8 数据
byteN-1,byteN CRC校验值
byte3==TYPE_STRING-> byte4字符串的显示方式,byte5字符串中字符的大小,byte6 byte7横坐标，byte8 byte9纵坐标,
*/

extern INT8U gFlashData[4096];
...
void UpdateProgramFromSD(void)
{
  LcdScreenClear();		
	...
	res=f_open(&fil,"data.bin",FA_OPEN_EXISTING|FA_READ);
	if(res==FR_OK)
	{
    LcdRAMShowStr(0,0,"找到data.bin",12);
		f_close(&fil);
		f_mount(0, NULL);
	}
  else
    LcdRAMShowStr(0,0,"找不到data.bin",14);
  
  res=f_open(&fil,"Pic0.bmp",FA_OPEN_EXISTING|FA_READ);
	if(res==FR_OK)
	{
    LcdRAMShowStr(1,0,"找到Pic0.bmp",12);
		f_close(&fil);
		f_mount(0, NULL);
	}
  else
    LcdRAMShowStr(1,0,"找不到Pic0.bmp",14);
	DelayXMs(1000);
}
//*_*这个函数有待优化
static
