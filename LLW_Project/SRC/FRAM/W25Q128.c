...
#include "W25Q128.h" 
#include "SPI.h"
u16 SPI_FLASH_TYPE=W25Q128; //W25Q128
...
{  
	...
	while(1)
	{   
		W25QxxWrite_Page(pBuffer,WriteAddr,pageremain);
		if(NumByteToWrite==pageremain)
			break;//
...
{ 
	...
	while(1) 
	{ 
		W25QxxRead(W25Qxx_BUF,secpos*4096,4096);//读出secpos扇区的数据，4096
		...
		{//里面有数据，需要擦除
			...
			W25QxxWrite_NoCheck(W25Qxx_BUF,secpos*4096,4096);//写入数据  
		}
		else//里面没有数据
