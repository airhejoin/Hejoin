...
{	
	...
	GPIO_InitStructure.GPIO_Pin = TOUCH_CLK_PIN |TOUCH_DOUT_PIN | TOUCH_DIN_PIN;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP;
	GPIO_Init(TOUCH_SPI_PORT, &GPIO_InitStructure);

	GPIO_SetBits(GPIOB, GPIO_Pin_4);//预置为高
...
{ 		   			 
	Pen_Point.Key_Sta=TOUCH_DOWN;//按键按下  		  				 
...
{								 
	...
	Pen_Point.Key_Sta=TOUCH_UP;//消除触发信号 
	Pen_Point.xfac=0;//xfac用来标记是否校准过,所以校准之前必须清掉!以免错误	 
	while(1)
	{
		if(Pen_Point.Key_Sta==TOUCH_DOWN)//按键按下了
		{
